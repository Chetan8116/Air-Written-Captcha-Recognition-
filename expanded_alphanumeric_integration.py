"""
Integration script for the expanded alphanumeric model
This module updates the original alphanumeric_model_integration.py to use the complete
...
class ExpandedAlphanumericModelLoader:
    """Class to load and manage the expanded alphanumeric model"""
    ...
    def load_model(self, model_path: str = "complete_alphanumeric_model.h5", 
                   labels_path: str = "complete_alphanumeric_labels.json") -> bool:
        """
        Load the trained expanded alphanumeric model and label mappings
        ...
        try:
            ...
            if not os.path.exists(model_path):
                ...
                # Try falling back to the original model
                model_path = "alphanumeric_model.h5"
...
expanded_model = ExpandedAlphanumericModelLoader()

def load_expanded_model() -> bool:
    """
    Load the expanded alphanumeric model (convenience function)
    
    Returns:
        bool: True if successfully loaded
    """
    return expanded_model.load_model()

def predict_expanded_character(image: np.ndarray) -> Tuple[str, float]:
    """
    ...
    Returns:
        Tuple[str, float]: (predicted_character, confidence)
    """
    return expanded_model.predict_character(image)

def get_expanded_predictions(image: np.ndarray, top_k: int = 3) -> list:
    """
    ...
    Returns:
        list: List of (character, confidence) tuples
    """
    return expanded_model.get_top_predictions(image, top_k)

def is_expanded_model_loaded() -> bool:
    """
    Check if expanded model is loaded
    
    Returns:
        bool: True if model is loaded
    """
    return expanded_model.is_loaded

# Test function
def test_expanded_model():
    """Test the expanded alphanumeric model with a sample image"""
    if not load_expanded_model():
        print("Failed to load expanded alphanumeric model for testing")
        return
    ...
    # Test prediction
    char, confidence = predict_expanded_character(test_image)
    print(f"Test prediction: '{char}' (confidence: {confidence:.3f})")
    
    # Test top predictions
    top_preds = get_expanded_predictions(test_image, top_k=5)
...
if __name__ == "__main__":
    ...
    print("=== Testing Expanded Alphanumeric Model Integration ===")
    test_expanded_model()